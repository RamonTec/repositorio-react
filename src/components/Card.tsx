...
const Card: React.FC<CardProps> = ({ imgSrc, title, description, technologies }) => {
  return (
    <div className="relative sm:w-1/2 md:w-80 lg:w-3/3 xl:w-3/3 p-4">
          ...
          <div className="absolute inset-0 bg-black bg-opacity-50 opacity-0 group-hover:opacity-100 transition-opacity">
            <div className="absolute inset-0 flex items-center justify-center">
              <div className="text-white p-4">
                ...
                <p className="mt-2 text-ellipsis text-gray-200">{description}</p>
                <div className="mt-4">
                  <h2 className="text-sm font-semibold text-gray-200">Technologies used:</h2>
                  <ul className="mt-1">
                    {technologies.map((tech, index) => (
                      <li key={index} className="text-xs text-gray-300">{tech}</li>
                    ))}
                  </ul>
                </div>
              </div>
